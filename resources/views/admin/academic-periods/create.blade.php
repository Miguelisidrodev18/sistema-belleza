<x-layouts.app title="Nuevo Período Académico">
    <x-slot:actions>
    ...
    <div class="max-w-2xl">
        <div class="rounded-xl border border-ugarte-border bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.academic-periods.store') }}">
                @csrf
                @include('admin.academic-periods._form', ['previousOptions' => $previousOptions])
            </form>
        </div>
    </div>
</x-layouts.app>
