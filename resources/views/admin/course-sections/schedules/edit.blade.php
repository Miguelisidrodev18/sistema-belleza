<x-layouts.app title="Editar Horario — {{ $courseSection->course->name }}">
    <div class="mx-auto max-w-xl rounded-xl border border-ugarte-border bg-white p-6 shadow-sm">
        <h2 class="mb-5 text-lg font-semibold text-gray-900">Editar Horario — {{ $courseSection->course->name }}</h2>

        <form action="{{ route('admin.course-sections.schedules.update', [$courseSection, $schedule]) }}" method="POST" class="space-y-4">
            @csrf @method('PUT')
            @include('admin.course-sections.schedules._form', ['editing' => true])
