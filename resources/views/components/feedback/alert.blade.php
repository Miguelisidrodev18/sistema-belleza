...
<div
    ...
    {{ $attributes->merge(['class' => 'rounded-lg border p-4 ' . ($typeMap[$type] ?? $typeMap['info'])]) }}
    role="alert"
>
    <div class="flex items-start gap-3">
        @if($icon)
            <div class="shrink-0 {{ $iconColors[$type] ?? $iconColors['info'] }}">
                @if($type === 'success')
        ...
        @if($dismissible)
            <button @click="show = false" class="shrink-0 opacity-50 transition-opacity hover:opacity-100">
